  ...
  try {
    ...
    if (!questions || !answers || questions.length !== answers.length) {
      return NextResponse.json(
        {
          error: "Questions and answers are required and must match in length",
        },
        { status: 400 }
      );
    }
    ...
    const feedbackPromises = questions.map(
      async (question: string, index: number) => {
        ...
        try {
          ...
          console.error(
            "Failed to parse Gemini response. Original text:",
            text,
            "Error:",
            parseError
          );
          return {
            question,
            user_answer: answer,
            feedback: `Failed to generate feedback: ${text}`,
            score: 0,
          };
        }
      }
    ...
    const overallScore =
      questionFeedbacks.length > 0 ? totalScore / questionFeedbacks.length : 0;
